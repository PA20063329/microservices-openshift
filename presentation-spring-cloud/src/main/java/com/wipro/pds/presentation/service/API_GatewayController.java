package com.wipro.pds.presentation.service;
...
import com.wipro.pds.presentation.model.Airport;
import com.wipro.pds.presentation.model.Flight;
import com.wipro.pds.presentation.model.FlightSegment;
import com.wipro.pds.presentation.model.Itinerary;
...
public class API_GatewayController
{
	...
	public String[] airports()
	{
		...
		logger.info( "Operation Look Up Airport Codes" );
...
//		tracer.addTag( "Operation", "Itinerary Query" );
		logger.info( "Operation Itinerary Query" );
...
//		querySpan.setBaggageItem( "forwarded-for", request.getHeader( "x-forwarded-for" ) );
		...
		logger.info("Fetching departing Flights by using URI = "+builder.toUriString());
		...
		else
		{
			...
			logger.info("Fetching return Flights by using URI = "+builder.toUriString());
	...
	private class PricingCall extends HystrixCommand<Itinerary>
	{
		...
		protected Itinerary run() throws Exception
		{
			try
			{
				logger.info("Posting request for fetching sales price by using URI = http://zuul/sales/price");
